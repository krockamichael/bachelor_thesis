...
-- standard local logger, expand by Tundrawolf only
reserved_tokens[def.LOGGER] = function(_, message)
	...
	ngx.log(ngx.ALERT, header .. ', ' .. (e_message or (type(e)=='string' and e or JSON_encode(e)) or 'no reason.'))
end
...
return {
	new = function(_, opt)
		...
		-- if <value> equ false in system_route table, then the <token> is invalid
		-- 	*) system_route_discoveries published
		local system_route_discoveries = {}
		system_route_discoveries["n4c.system.discoveries"] = function() return system_route_discoveries end

		local function route_discovery(token)
			if system_route_discoveries[token] then
				local ok, result = pcall(system_route_discoveries[token], token)
				if ok then return result end
			end
		end

		local function system_route(token)
			return options.system_route[token] or route_discovery(token)
		end
