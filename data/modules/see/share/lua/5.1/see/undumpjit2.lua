...
local function bcread_header(ls, target)
    local first = bcread_byte(ls)
    local second = bcread_byte(ls)
    local third = bcread_byte(ls)
    if first ~= BCDUMP.HEAD2 or second ~= BCDUMP.HEAD3 or third ~= BCDUMP.VERSION then
...
local function bcread(s, output, chunkname, hexdump)
    ...
    local meh = bcread_byte(ls)
    if meh ~= BCDUMP.HEAD1 then
        return "invalid header beginning char"
    end
    bcread_header(ls, printer)
    local pt, last_pt
    repeat
        pt = bcread_proto(ls, printer)
        if pt then last_pt = pt end
...
local function undump(str_or_function)
    ...
    local proto = bcread(str, {write = function() end})
    return setmetatable(
        proto,
        {
