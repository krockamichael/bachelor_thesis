...
iterate_test_data('test_suite/aliases_positive', function (data)
    local validator = livr.new(data.rules)
    for i = 1, #data.aliases do
        validator:register_aliased_rule(data.aliases[i])
    end
    local output, err = validator:validate(data.input)
    nok( err, "should return no errors" )
    if err then
...
iterate_test_data('test_suite/aliases_negative', function (data)
    local validator = livr.new(data.rules)
    for i = 1, #data.aliases do
        validator:register_aliased_rule(data.aliases[i])
    end
