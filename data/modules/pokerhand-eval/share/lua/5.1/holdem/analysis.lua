...
local function card_to_binary_lookup5(card) -- luacheck: ignore
  return lookup.Tables.Five.card_to_binary[card.rank + 1][card.suit + 1]
end

local function card_to_binary_lookup6(card) -- luacheck: ignore
  return lookup.Tables.Six.card_to_binary[card.rank + 1][card.suit + 1]
end

local function card_to_binary_lookup7(card) -- luacheck: ignore
...
local function to_cards(h, b)
  local c = {}
  __.each({h, b}, function (t)
    __.each(t, function (a)
      table.insert(c, a)
    end)
  end)
  return c
...
function Analysis.evaluate(hand, board)
  ...
  assert(#hand == 2, "Hand size not valid!")
  ...
  elseif #cards == 7 then
    ...
    error("invalid hand/board (must have 2, 5, 6 or seven cards) size == "..#cards)
  end
